...
#############################################
# Forum Checker Proxy Setup Script v1.0
# Sets up HTTP Anonymous Proxy
# Using 3proxy on Ubuntu/Debian
# Auto-detects NAT environments (AWS, etc.)
#
# Optimized for: forum-registration-checker
# (NOT for Xrumer/XEvil - different priorities)
#
# KEY DIFFERENCES vs proxy.sh (Xrumer edition):
# - tcp_syn_retries: 1 -> 3
#     Xrumer'da zombie thread temizlemek için 1'di.
#     Forum checker'da daha fazla retry = daha az hatalı "error" sonucu.
#
# - ClientIdle: 600s -> 60s
#     XEvil captcha çözümü için 600s gerekiyordu.
#     Forum checker'da client (Rust programı) idle kalmaz, gerek yok.
#
# - ServerIdle: 600s -> 60s
#     Aynı sebep. Forum sadece 30-60s içinde cevap vermeli.
#
# - ServerRead/Write: 110s -> 60s
#     Xrumer'ın 100s POST limiti için 110s'di.
#     Forum checker basit GET isteği yapıyor, 60s yeterli.
#
# - maxconn: 5000 (aynı - 500 concurrent için yeterli)
#
# TIMEOUT TABLOSU:
# ClientRead ClientWrite ServerRead ServerWrite ServerConnect ClientIdle Resolve ServerIdle
#     10          10         60          60           30          60       15       60
#############################################
...
print_header() {
    clear
    echo -e "${CYAN}╔════════════════════════════════════╗${NC}"
    echo -e "${CYAN}║${NC}   ${BOLD}Forum Checker Proxy Setup v1.0${NC}   ${CYAN}║${NC}"
    echo -e "${CYAN}║${NC}      HTTP Anonymous Proxy          ${CYAN}║${NC}"
    echo -e "${CYAN}║${NC}   forum-registration-checker       ${CYAN}║${NC}"
    echo -e "${CYAN}╚════════════════════════════════════╝${NC}"
    echo ""
}
...
confirm_installation() {
    ...
    echo "  ClientIdle   : 60s (forum checker optimized, no XEvil)"
    echo "  ServerIdle   : 60s (forum checker optimized)"
    echo "  tcp_syn_retries: 3 (more retries = fewer false errors)"
    echo ""
...
# Forum Checker Proxy v1.0 - forum-registration-checker Optimized
...
# SYN retries: 3 (vs Xrumer'daki 1)
# Xrumer'da 1'di: zombie thread'leri hızlı öldürmek için.
# Forum checker'da 3 daha iyi: geçici ağ sorunlarında bağlantı kopmaz,
# "error" sayısı azalır. Timeout süresi: ~45s (kabul edilebilir).
...
configure_3proxy() {
    ...
    cat > "$PROXY_CFG" <<EOF
# 3proxy configuration - Forum Checker Proxy v1.0
# Optimized for: forum-registration-checker (NOT Xrumer/XEvil)
...
# ClientRead ClientWrite ServerRead ServerWrite ServerConnect ClientIdle Resolve ServerIdle
#
# ClientRead  (10s): Rust programı isteği proxy'e gönderir (anlık, 10s yeterli)
# ClientWrite (10s): Proxy cevabı Rust programına gönderir (lokal, hızlı)
# ServerRead  (60s): Forum sayfasının yüklenmesini bekle (Xrumer'da 110s'di - gerek yok)
# ServerWrite (60s): Forum'a veri gönder (Xrumer'da 110s'di - gerek yok)
# ServerConnect(30s): Forum ile TCP handshake (yavaş ama açık siteleri kapsamak için)
# ClientIdle  (60s): Rust programı idle kalmaz (Xrumer'da 600s'di - XEvil için)
# Resolve     (15s): Yavaş DNS sunucuları için
# ServerIdle  (60s): Forum idle kalmaz (Xrumer'da 600s'di - XEvil için)
timeouts 10 10 60 60 30 60 15 60
...
[Unit]
Description=3proxy Proxy Server (Forum Checker Edition v1.0)
...
═══════════════════════════════════════
Forum Checker Proxy Server v1.0
═══════════════════════════════════════
...
Optimizations for forum-registration-checker:
✓ nf_conntrack_max=262144 (ROOT FIX - no more packet drops)
✓ tcp_syn_retries=3 (more retries = fewer false "error" results)
✓ nserver removed (OS handles DNS, no bottleneck)
✓ ClientIdle=60s (no XEvil - Rust program doesn't idle)
✓ ServerIdle=60s (forums respond within 60s or they're down)
✓ ServerRead/Write=60s (simple GET requests, no 100s POST needed)
...
#!/bin/bash
echo "╔════════════════════════════════════╗"
echo "║   3proxy Status Monitor v1.0       ║"
echo "║   Forum Checker Edition            ║"
echo "╚════════════════════════════════════╝"
echo "Timestamp: $(date)"
echo ""
...
display_results() {
    ...
    print_separator
    echo ""
    echo -e "${BOLD}${GREEN}Optimizations in v1.0 (Forum Checker):${NC}"
    echo "  ✓ nf_conntrack_max=262144 (ROOT FIX - silent packet drops eliminated)"
    echo "  ✓ tcp_syn_retries=3 (more retries = fewer false 'error' results)"
    echo "  ✓ DNS bottleneck removed (OS handles DNS, not 3proxy)"
    echo "  ✓ ClientIdle=60s (no XEvil needed)"
    echo "  ✓ ServerIdle=60s (forums respond within 60s)"
    echo "  ✓ ServerRead/Write=60s (simple GET, no long POST needed)"
